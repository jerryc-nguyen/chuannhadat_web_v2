...
const LocationForm: React.FC<ILocationForm> = ({ form }) => {
  ...
  useEffect(() => {
    ...
    if (!city_id) {
      setFullAddressInfo((prev) => {
        return { ...prev, cityLabel: '', districtLabel: '' };
      });
    } else {
      setFullAddressInfo((prev) => {
  ...
  const constructFullAddress = useCallback(
    (props: {
      projectLabel?: string;
      streetLabel?: string;
      wardLabel?: string;
  ...
  return (
    <Card className="bg-primary/10">
      ...
      <CardContent className="grid gap-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
              ...
              <FormItem className="grid gap-2">
                ...
                <AutoComplete
                  ...
                  onSelectedValueChange={(value) => {
                    ...
                    setFullAddressInfo((prev) => {
                      return { ...prev, cityLabel: value?.label || '' };
                    });
                  }}
              ...
              <FormItem className="grid gap-2">
                ...
                <AutoComplete
                  ...
                  onSelectedValueChange={(value) => {
                    field.onChange(value?.value || '');
                    setFullAddressInfo((prev) => {
                      return { ...prev, districtLabel: value?.label || '' };
                    });
                  }}
                  items={
                    city_id
              ...
              <FormItem className="grid gap-2">
                ...
                <AutoComplete
                  ...
                  onSelectedValueChange={(value) => {
                    field.onChange(value?.value || '');
                    setFullAddressInfo((prev) => {
                      return { ...prev, wardLabel: value?.label || '' };
                    });
                  }}
                  items={
                    district_id
              ...
              <FormItem className="grid gap-2">
                ...
                <AutoComplete
                  ...
                  onSelectedValueChange={(value) => {
                    field.onChange(value?.value || '');
                    setFullAddressInfo((prev) => {
                      return { ...prev, streetLabel: value?.label || '' };
                    });
                  }}
                  items={
                    district_id
              ...
              <FormItem className="grid gap-2">
                ...
                <AutoComplete
                  ...
                  onSelectedValueChange={(value) => {
                    field.onChange(value?.value || '');
                    setFullAddressInfo((prev) => {
                      return { ...prev, projectLabel: value?.label || '' };
                    });
                  }}
                  items={
                    district_id
          ...
          <div className="grid gap-2">
            ...
            <Input
              id="subject"
              placeholder="Nhập Địa chỉ..."
              value={fullAddressInfo.addressText}
              onChange={(e) =>
                setFullAddressInfo((prev) => {
                  return { ...prev, addressText: e.target.value };
                })
              }
            />
          </div>
        ...
        <div className="flex flex-col gap-y-5 rounded-lg border border-solid border-[#c8c8cd] bg-secondary p-2">
          <div className="flex w-full items-center justify-between">
            <div className="flex items-center gap-1">
              <CircleAlert />
              <span className="text-grey-900 font-medium">Địa chỉ hiển thị trên tin đăng</span>
            </div>
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(value) => {
                  setIsHideFullAddress(value as boolean);
                }}
              />
              <label
                htmlFor="terms"
          ...
          <span className="text-grey-900 text-fs-14">{fullAddress}</span>
        </div>
      </CardContent>
    </Card>
  );
